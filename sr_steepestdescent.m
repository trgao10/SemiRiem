function [x, cost, info, options] = sr_steepestdescent(problem, x, options)
% Semi-Riemannian steepest descent (gradient descent) minimization algorithm.
%
...
% Modified from steepestdescent.m, which is part of Manopt: www.manopt.org.
...
%       Now using storedb.remove() to keep the cache lean.
    ...
    [cost, srgrad] = getSRCostGrad(problem, x, storedb, key);
    [grad, gradnorm] = problem.M.getDescDir(x, srgrad, options.deterministic);
%     if options.deterministic
% %         [rcost, rgrad] = getCostGrad(problem, x, storedb, key);
%         [cost, srgrad] = getSRCostGrad(problem, x, storedb, key);
%         
%         grad = srgrad;
% %         grad = problem.M.sigMat * srgrad;
%         
%         gradnorm = sqrt(problem.M.inner(x, grad, srgrad));
%     else
%         [cost, srgrad] = getSRCostGrad(problem, x, storedb, key);
%         orthobasis = sr_tangentorthobasis(problem.M, x);
%         gradcoeff = problem.M.inner(x, orthobasis, srgrad);
%         grad = orthobasis*gradcoeff;
%         gradnorm = norm(gradcoeff);
%     end
...
%         fprintf(' iter\t cost val\t x\t                             grad.norm\t grad\n');
%         fprintf(' iter\t cost val\t x\t grad.norm\t grad\n');
        fprintf(' iter\t cost val\t grad.norm\t grad\n');
    end
    ...
    while true
        ...
        if options.verbosity >= 2
%             fprintf('%5d\t %+.2e\t (%.2e, %.2e, %.2e)\t%.8e\t(%.2e, %0.2e)\n', iter, cost, x, gradnorm, grad(1:2));
            fprintf('%5d\t%+.2e\t%.8e\t(%.2e, %0.2e)\n', iter, cost, gradnorm, grad(1:2));
%             fprintf('%5d\t%+.16e\t%.8e\n', iter, cost, gradnorm);
        end
        ...
        [newcost, newsrgrad] = getSRCostGrad(problem, newx, storedb, newkey);
        [newgrad, newgradnorm] = problem.M.getDescDir(newx, newsrgrad, options.deterministic);
%         if options.deterministic
% %             [cost, grad] = getCostGrad(problem, newx, storedb, newkey);
%             [newcost, newsrgrad] = getSRCostGrad(problem, newx, storedb, newkey);
%             
%             newgrad = newsrgrad;
% %             newgrad = problem.M.sigMat * newsrgrad;
% 
%             newgradnorm = sqrt(problem.M.inner(newx, newgrad, newsrgrad));
%         else
%             [newcost, newsrgrad] = getSRCostGrad(problem, newx, storedb, newkey);
%             orthobasis = sr_tangentorthobasis(problem.M, newx);
%             newgradcoeff = problem.M.inner(newx, orthobasis, newsrgrad);
%             newgrad = orthobasis*newgradcoeff;
%             newgradnorm = norm(newgradcoeff);
%         end
