function [x, cost, info, options] = sr_conjugategradient(problem, x, options)
...
gradPgrad = M.inner(x, srgrad, Pgrad);
...
%     if options.deterministic
%         [newcost, newsrgrad] = getSRCostGrad(problem, newx, storedb, newkey);
%         newgrad = M.sigMat * newsrgrad;
%         newgradnorm = sqrt(M.inner(newx, newsrgrad, newgrad));
%     else
%         [newcost, newsrgrad] = getSRCostGrad(problem, newx, storedb, newkey);
%         orthobasis = sr_tangentorthobasis(problem.M, newx);
%         newgradcoeff = problem.M.inner(x, orthobasis, newsrgrad);
...
%     newgradnorm = M.norm(newx, newgrad);
    ...
    newgradPnewgrad = M.inner(newx, newsrgrad, Pnewgrad);
%     newgradPnewgrad = M.inner(newx, newgrad, Pnewgrad);
    ...
    if strcmpi(options.beta_type, 'steep') || ...
       strcmpi(options.beta_type, 'S-D')              % Gradient Descent
        ...
        oldsrgrad = M.transp(x, newx, srgrad);
        orth_grads = M.inner(newx, oldsrgrad, Pnewgrad) / newgradPnewgrad;
        ...
        if abs(orth_grads) >= options.orth_value
            ...
            switch upper(options.beta_type)
                ...
                case 'P-R'  % Polak-Ribiere+
                    diff = M.lincomb(newx, 1, newsrgrad, -1, oldsrgrad);
