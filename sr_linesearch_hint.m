function [stepsize, newx, newkey, lsstats] = ...
             sr_linesearch_hint(problem, x, d, f0, df0, options, storedb, key)
...
%         keyboard
    ...
    flipped_d_coeff = problem.M.inner(x, orthobasis, d);
    norm_d = norm(flipped_d_coeff);
%     norm_d = sqrt(flipped_d'*orthosig*d);
%     [orthobasis, orthosig] = sr_tangentorthobasis(problem.M, x);
%     flipped_d = orthobasis*orthobasis'*orthosig*d;
%     norm_d = sqrt(flipped_d'*orthosig*d);
%     norm_d = sqrt(problem.M.inner(x, d, flipped_d));
